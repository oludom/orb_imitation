...
from datagen.RecurrentTrackLoader import RecurrentTracksDataset
...
def load_dataset_train_val_split(dataset_basepath, dataset_basename, device, maxTracks, input_channels,
    ...
    return datasets
def load_dataset_train_val_split_recurrent(dataset_basepath, dataset_basename, device, maxTracks, input_channels,
                 skipFirstXImages, skipLastXImages, batch_size, tf, jobs=1):
...
def get_path_for_run(project_basepath, dataset_basename, itypes, resnet_factor, batch_size, loss_type, learning_rate, TB_suffix, beta_rate):
    TB_path = Path(project_basepath,
                ...
                f"_bs={batch_size}_lt={loss_type}_lr={learning_rate}beta_rate={beta_rate}__c={TB_suffix}_train_dung")
    if TB_path.exists():
...
def get_path_for_run_rnn(project_basepath, dataset_basename, itypes, resnet_factor, batch_size, loss_type, learning_rate, TB_suffix, beta_rate):
    TB_path = Path(project_basepath,
                f"runs_rgbdo/RaceNet8_ds={dataset_basename}_l={itypes}_f={resnet_factor}"
                f"_bs={batch_size}_lt={loss_type}_lr={learning_rate}beta_rate={beta_rate}__c={TB_suffix}_betafix")
    if TB_path.exists():
        print("TB_path exists")
        exit(0)
    writer = SummaryWriter(str(TB_path))
...
def train_epoch_recurrent(epoch, epochs, model, phases, 
                        learning_rate, learning_rate_change, learning_rate_change_epoch, 
                        datasets, dev, lossfunction, writer, step_pos, TB_path, batch_count
...
def train(project_basepath,
    ...
    datasets = load_dataset_train_val_split(dataset_basepath,dataset_basename,device,
                                                    1000,
                                                    input_channels, skipFirstXImages,
                                                    skipLastXImages, batch_size, tf, jobs)
    ...
    try:
        for epoch in range(epochs):
            # train epoch
            model = train_epoch(epoch, epochs, model, phases, learning_rate, learning_rate_change, learning_rate_change_epoch, datasets, dev, lossfunction, writer, step_pos, TB_path, batch_count)
